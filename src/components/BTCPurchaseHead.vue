  ...
  <div class="card">
    <div class="card-header p-1">
      <div class="grid grid-cols-3 items-center my-2">
        ...
        <div class="flex flex-col xs:flex-row justify-center items-center">
          <img
            src="../assets/bitcoin-purchase.svg"
            alt="bitcoin-purchase"
            class="icon"
          />
          <p v-if="this.currency == 'eur'" :class="BTCClass" class="mx-2">
            {{ currencydecimal(this.total.btc.eur, 1) }} €
          </p>
          <p v-if="this.currency == 'usd'" :class="BTCClass" class="mx-2">
            {{ currencydecimal(this.total.btc.usd, 1) }} $
          </p>
        </div>
      </div>
    </div>
    ...
    >
      <div class="flex justify-center items-center">
        ...
        >
          <p class="white-bold-text">ADD BTC</p>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    buyBTC() {
      ...
      this.$store.commit("addBitcoinPurchase", {
